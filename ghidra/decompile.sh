...
mkdir -p "$OUTDIR"
echo "[*] Exporting decompilation of 'ctfnode.stripped' to $OUTDIR"

# -process ctfnode.stripped : the program inside the project to work on
# -noanalysis             : do not re-analyse (it is already analysed)
# -postScript ExportDecomp.java <dir> : run our export script after loading
"$GH/support/analyzeHeadless" "$HERE/proj" CTFNodeRE \
    -process ctfnode.stripped -noanalysis \
    -scriptPath "$HERE" \
